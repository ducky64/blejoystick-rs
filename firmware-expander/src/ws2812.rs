...
/// WordsPerColor is the maximum number of SPI words used to encode a single color of LED data
///   This encoding is needed for internal array sizing without generic const exprs
pub struct Ws2812SpiCustom<Word, SPI, const N: usize, const WordsPerColor: usize> {
    spi: SPI,
    zero: Word,
    zero_bits: u8,
    one: Word,
    one_bits: u8,
    buffer: [[[Word; WordsPerColor]; 3]; N],
}

impl <Word: Copy + 'static, SPI, const N: usize, const WordsPerColor: usize> Ws2812SpiCustom<Word, SPI, N, WordsPerColor> 
where
    SPI: SpiBus<Word>
{
    ...
    pub fn new(spi: SPI, zero: Word, zero_bits: u8, one: Word, one_bits: u8) -> Self {
        Self { spi, zero, one, buffer: [[[0.into(); WordsPerColor]; 3]; N]}
    }
}

impl <Word, SPI, const N: usize, const WordsPerColor: usize> SmartLedsWriteAsync
for Ws2812SpiCustom<Word, SPI, N, WordsPerColor> 
where
    SPI: SpiBus<Word>,
    Word: Copy + From<u8> + Shl<u8, Output = Word> + Shr<u8, Output = Word> + BitOr<Word, Output = Word> + NumBits + 'static,
{
        ...
        I: Into<Self::Color>
    {
        ...
        for item in iterator {
            ...
            for color_byte in [item.g, item.r, item.b] {
                for bit in (0..8).rev() {
                    ...
                    let shift_bits = word_bits % Word::BITS;
        ...
        if word_bits > 0 {
            self.buffer[buffer_index] = word_buffer << (Word::BITS - word_bits);
            buffer_index += 1;
        }
